...
<nav class="fixed z-50 flex h-20 w-screen flex-row items-center justify-between bg-gradient-to-b from-black via-black to-transparent px-4">
	...
	<ul class="desktop-nav hidden md:flex justify-around h-full w-1/2 items-center space-x-8">
		<li>
            <a href="#projects" class="text-white sm:text-lg lg:text-xl xl:text-2xl">
                projects
            </a>
        </li>
		<li>
	...
	<!-- Mobile Menu Button -->
	<button class="md:hidden text-white text-3xl focus:outline-none" on:click={() => (isOpen = !isOpen)}>
		{#if isOpen}
			&times;
		{:else}
			&#9776;
